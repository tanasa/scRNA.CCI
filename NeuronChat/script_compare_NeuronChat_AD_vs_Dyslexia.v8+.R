# ══════════════════════════════════════════════════════════════════
# NEURONCHAT COMPARISON ANALYSIS  (script v8+)
# Comparison : AD vs Dyslexia
# Cond 1     : AD       (positive reference → logFC > 0 = higher in AD)
# Cond 2     : Dyslexia
# v8: per-pathway plots use pheatmap(delta) only — see pathway_delta_pheatmap/
# v8+: subset_neuronchat keeps @idents length = ncol(@data) when dropping rare cell types (mergeNeuronChat-safe)
# Outputs → compare_NeuronChat_*_*_v8+/ on NFS
# ══════════════════════════════════════════════════════════════════
...
rds1 <- file.path(base_dir,
  "18_merged_AG_031826.28celltypes.AD_neuronchat",
  "neuronchat_18_merged_AG_031826.28celltypes.AD_at_the_end_v5.rds")

rds2 <- file.path(base_dir,
  "18_merged_AG_031826.28celltypes.Dyslexia_neuronchat",
  "neuronchat_18_merged_AG_031826.28celltypes.Dyslexia_at_the_end_v5.rds")

work_dir   <- file.path(base_dir,
  paste0("compare_NeuronChat_", name1, "_vs_", name2, "_v8+"))
...
log_file   <- file.path(work_dir,
  paste0("neuronchat_compare_", name1, "_vs_", name2, "_v8+_",
         format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
start_time <- Sys.time()
sink(log_file, split = TRUE)

cat("╔═══════════════════════════════════════════════════════════╗\n")
cat(sprintf("║  NeuronChat Comparison v8+: %s vs %s\n", name1, name2))
cat("╚═══════════════════════════════════════════════════════════╝\n")
...
if (length(only1) > 0 || length(only2) > 0) {
  ...
  # Subset @net to common cell-type rows/cols; align @data, @idents, @meta
  # (must compute keep_cells from *original* idents — idents length must match ncol(@data))
  subset_neuronchat <- function(obj, keep_ct) {
    ...
    bc <- colnames(obj@data)
    keep_cells <- bc[as.character(obj@idents) %in% keep_ct]
    if (length(keep_cells) > 0) {
      obj@data <- obj@data[, keep_cells, drop = FALSE]
      if (!is.null(obj@data.signaling) && ncol(obj@data.signaling) > 0) {
        sig_ok <- intersect(keep_cells, colnames(obj@data.signaling))
        obj@data.signaling <- obj@data.signaling[, sig_ok, drop = FALSE]
      }
      if (!is.null(obj@meta) && nrow(obj@meta) > 0 &&
          length(rownames(obj@meta)) > 0) {
        obj@meta <- obj@meta[intersect(rownames(obj@meta), keep_cells), , drop = FALSE]
      }
      ix <- match(keep_cells, bc)
      obj@idents <- factor(as.character(obj@idents)[ix], levels = keep_ct)
    } else {
      obj@idents <- factor(character(0), levels = keep_ct)
    }
    obj
  }
...
if (!is.null(neuronchat_list)) {
  ...
  # ── Per-pathway delta heatmaps (pheatmap: color = delta only; no dot / mean-weight size) ──
  cat("\n--- Per-Pathway Delta Heatmaps (pheatmap) ---\n")
  delta_hm_dir <- file.path(output_dir, "pathway_delta_pheatmap")
  dir.create(delta_hm_dir, showWarnings = FALSE)
  ...
  for (pw in all_pathways) {
    ...
    if (nrow(df_pw) == 0 || all(df_pw$mean_w == 0)) next
    senders   <- sort(unique(df_pw$sender))
    receivers <- sort(unique(df_pw$receiver))
    diff_mat  <- matrix(0, length(senders), length(receivers),
                        dimnames = list(senders, receivers))
    for (k in seq_len(nrow(df_pw))) {
      diff_mat[df_pw$sender[k], df_pw$receiver[k]] <- df_pw$delta[k]
    }
    max_d <- max(abs(diff_mat), na.rm = TRUE)
    if (is.na(max_d) || max_d == 0) next
    pw_clean <- gsub("[^A-Za-z0-9_-]", "_", pw)
    tryCatch({
      png(file.path(delta_hm_dir, paste0(pw_clean, "_delta_pheatmap.png")),
          width = 1600, height = 1200, res = 150, bg = "white")
      pheatmap::pheatmap(
        diff_mat,
        main = sprintf("NeuronChat: %s (%s − %s)\ncolor = delta (no mean-weight sizing)",
                       pw, name2, name1),
        color = grDevices::colorRampPalette(c(col1, "white", col2))(100),
  ...
  cat("\n✓ Per-pathway delta pheatmap plots saved → ", delta_hm_dir, "\n", sep = "")
  ...
  saveRDS(neuronchat_list,
          file.path(work_dir,
                    sprintf("neuronchat_merged_%s_vs_%s_v8+.rds",
                            name1, name2)))
...
cat("\n")
cat("╔═══════════════════════════════════════════════════════════╗\n")
cat(sprintf("║  ✓✓✓  NeuronChat %s vs %s COMPLETE (v8+)  ✓✓✓\n",
            name1, name2))
cat("╚═══════════════════════════════════════════════════════════╝\n")
