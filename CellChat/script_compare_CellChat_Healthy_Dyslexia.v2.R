...
run_healthy_perspective <- FALSE   # ← set TRUE to also run Healthy perspective
...
p2 <- ggplot() +
  ...
  scale_color_manual(values = c("Healthy"  = "#4DAF4A",
                                 "Dyslexia" = "#377EB8")) +
  theme_classic(base_size = 13) +
  labs(title = "Functional Shift (Healthy → Dyslexia)", color = "Condition") +
  coord_fixed()
...
p2_s <- ggplot() +
  ...
  scale_color_manual(values = c("Healthy"  = "#4DAF4A",
                                 "Dyslexia" = "#377EB8")) +
  theme_classic(base_size = 13) +
  labs(title = "Structural Shift (Healthy → Dyslexia)", color = "Condition") +
  coord_fixed()
...
cat("logFC > 0 = higher in Dyslexia vs Healthy\n")
cat("logFC < 0 = lower  in Dyslexia vs Healthy\n\n")
...
if (deg_success_Dyslexia) {
  ...
  write.csv(net.up,
            ...
            row.names = FALSE)
  write.csv(net.down,
            "results.netMappingDEG_downregulated_Dyslexia.datasets_Healthy.ligandlogFC_-0.05.receptorlogFC_-0.05.csv",
            row.names = FALSE)
  cat("✓ Saved: results.netMappingDEG_upregulated_Dyslexia.datasets_Dyslexia.ligandlogFC_0.05.receptorlogFC_0.05.csv\n")
  cat("✓ Saved: results.netMappingDEG_downregulated_Dyslexia.datasets_Healthy.ligandlogFC_-0.05.receptorlogFC_-0.05.csv\n")
  ...
  write.csv(net.down.strict,
            "results.netMappingDEG_downregulated_Dyslexia.datasets_Healthy.ligandlogFC_-0.25.receptorlogFC_-0.25.csv",
            row.names = FALSE)
  cat("✓ Saved: results.netMappingDEG_upregulated_Dyslexia.datasets_Dyslexia.ligandlogFC_0.25.receptorlogFC_0.25.csv\n")
  cat("✓ Saved: results.netMappingDEG_downregulated_Dyslexia.datasets_Healthy.ligandlogFC_-0.25.receptorlogFC_-0.25.csv\n")
  ...
  tryCatch({
    gg_up <- netVisual_bubble(cellchat,
                               ...
                               angle.x        = 90,
                               remove.isolate = TRUE,
                               title.name     = "Up-regulated in Dyslexia vs Healthy (logFC > 0.05)")
  ...
  tryCatch({
    gg_down <- netVisual_bubble(cellchat,
                                 ...
                                 angle.x        = 90,
                                 remove.isolate = TRUE,
                                 title.name     = "Down-regulated in Dyslexia vs Healthy (logFC < -0.05)")
  ...
  tryCatch({
    gg_up_strict <- netVisual_bubble(cellchat,
                                      ...
                                      angle.x        = 90,
                                      remove.isolate = TRUE,
                                      title.name     = "Up-regulated in Dyslexia vs Healthy (logFC > 0.25)")
  ...
  tryCatch({
    gg_down_strict <- netVisual_bubble(cellchat,
                                        ...
                                        angle.x        = 90,
                                        remove.isolate = TRUE,
                                        title.name     = "Down-regulated in Dyslexia vs Healthy (logFC < -0.25)")
...
if (run_healthy_perspective) {
  ...
  cat("logFC > 0 = higher in Healthy vs Dyslexia\n")
  cat("logFC < 0 = lower  in Healthy vs Dyslexia\n\n")
  ...
  if (deg_success_Healthy) {
    ...
    tryCatch({
      gg_down_H <- netVisual_bubble(cellchat,
                                     ...
                                     angle.x        = 90,
                                     remove.isolate = TRUE,
                                     title.name     = "Down-regulated in Healthy vs Dyslexia")
...
detected_pathways <- unique(c(healthy@netP$pathways, dyslexia@netP$pathways))
